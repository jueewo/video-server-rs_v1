        ...
        <div class="w-screen flex overflow-x-auto gap-4 p-4 bg-surface-alt rounded-xl my-9 scrollbar-thin">
          {loadeddata.map((newsitem, index) => (
            <div key={index} class="flex-shrink-0 w-4/5">
              <div
                id={`item-${index}`}
                class={`card-ventures h-64 w-full bg-gradient-to-r from-primary to-blue-900 text-white flex ${newsitem.img ? "flex-row" : ""}`}
              >
                {newsitem.img && (
                ...
                <div class="p-5 flex flex-col justify-between flex-1">
                  ...
                  <div class="flex justify-end">
                    <a href={newsitem.link}>
                      <button class="btn-ventures bg-white/20 hover:bg-white/30 text-white">{newsitem.button} &#10095;</button>
                    </a>
                  </div>
                </div>
              </div>
